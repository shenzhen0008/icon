    ...
    <ul class="mt-4 space-y-3">
      @foreach ($products as $product)
        <li class="rounded-xl border border-theme bg-theme-secondary/20 p-4">
          ...
          @else
            <a href="/products/{{ $product['id'] }}" class="text-fluid-action mt-3 inline-flex h-[clamp(1.9rem,7vw,2.2rem)] w-[clamp(8rem,45vw,10rem)] items-center justify-center rounded-lg px-[clamp(0.6rem,2.5vw,0.9rem)] font-semibold {{ $product['is_eligible'] ? 'bg-[rgb(var(--theme-primary))] text-theme-on-primary' : 'bg-theme-secondary text-theme' }}">
