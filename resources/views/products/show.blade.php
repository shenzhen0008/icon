  ...
  <main class="mx-auto w-full max-w-3xl px-4 pb-28 pt-6 md:pb-10 md:pt-8">
    ...
    <section class="overflow-hidden rounded-3xl border border-theme bg-theme-card p-2.5 text-theme shadow-xl shadow-[rgb(var(--theme-primary))]/10">
      <div class="flex flex-wrap items-center justify-between gap-3">
        <div class="flex min-w-0 items-center gap-2">
          <div class="flex h-8 w-8 shrink-0 items-center justify-center overflow-hidden rounded-full border border-theme bg-theme-secondary/70 text-theme-secondary">
          ...
          <h1 class="truncate text-xl font-semibold">{{ $product['name'] }}</h1>
        </div>
        @if ($product['purchase_limit'] !== null)
          <p class="shrink-0 text-[11px] text-theme-secondary">限购 <span class="font-semibold text-[rgb(var(--theme-primary))]">{{ $product['purchase_limit'] }}</span> 份</p>
        @endif
      </div>
      ...
      <div class="mt-3.5 rounded-2xl border border-theme bg-theme-secondary/70 px-2 py-1.5">
        <div class="flex flex-nowrap items-center gap-2 overflow-x-auto overflow-y-hidden whitespace-nowrap pb-1 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          @foreach ($product['symbol_icon_paths'] as $iconPath)
            <span class="flex h-7 w-7 shrink-0 items-center justify-center overflow-hidden rounded-full border border-theme bg-theme-card text-theme-secondary">
    ...
    <section class="mt-6 rounded-2xl border border-theme bg-theme-card p-6">
      ...
      @if ($isGuest)
        ...
        <a href="/login" class="mt-4 inline-flex rounded-lg bg-[rgb(var(--theme-primary))] px-4 py-2 text-sm font-semibold text-theme-on-primary">去登录</a>
      @else
