...
<body class="min-h-screen bg-theme text-theme">
  ...
  <main class="mx-auto w-full max-w-7xl px-4 pb-28 pt-8 md:pb-10">
    <section class="rounded-3xl border border-[rgb(var(--theme-primary))]/20 bg-gradient-to-br from-[rgb(var(--theme-primary))]/10 to-[rgb(var(--theme-accent))]/10 p-6 shadow-xl shadow-[rgb(var(--theme-primary))]/10">
      ...
      <p class="mt-2 text-sm text-theme-secondary">请选择币种后向对应地址付款，完成后上传付款截图，管理员将手动核实并入账。</p>

      @if (count($assets) === 0)
        <div class="mt-6 rounded-xl border border-[rgb(var(--theme-rose))]/40 bg-[rgb(var(--theme-rose))]/10 p-3 text-sm text-theme">
          收款配置缺失，请联系管理员。
        </div>
      @else
        <div class="mt-6 rounded-2xl border border-theme bg-theme-card p-4">
          <p class="text-sm text-theme-secondary">收款通道</p>
          <div class="mt-3 flex flex-wrap gap-2" id="asset-selector" role="tablist" aria-label="币种选择">
            @foreach ($assets as $asset)
              ...
              <button
                ...
                class="rounded-lg border px-3 py-1.5 text-sm font-medium transition {{ $assetCode === $selectedAssetCode ? 'border-[rgb(var(--theme-primary))] bg-[rgb(var(--theme-primary))]/15 text-[rgb(var(--theme-primary))]' : 'border-theme text-theme-secondary hover:border-[rgb(var(--theme-primary))]/40 hover:text-theme' }}"
              >
                {{ $assetCode }}
              </button>
            @endforeach
        ...
        <div class="mt-4 grid gap-4 lg:grid-cols-2">
          <div class="rounded-2xl border border-theme bg-theme-card p-4">
            <p class="text-sm text-theme-secondary">当前币种</p>
            <p class="mt-2 text-lg font-semibold text-[rgb(var(--theme-primary))]" id="asset-code">{{ $selectedAsset['code'] ?? '--' }}</p>
            ...
            <button id="copy-wallet-address" class="mt-4 rounded-lg border border-[rgb(var(--theme-primary))]/40 px-4 py-2 text-sm text-[rgb(var(--theme-primary))] hover:bg-[rgb(var(--theme-primary))]/10">
              复制收款地址
            </button>
            <p id="copy-feedback" class="mt-2 hidden text-xs text-[rgb(var(--theme-primary))]">已复制</p>
          </div>

          <div class="rounded-2xl border border-theme bg-theme-card p-4">
            <p class="text-sm text-theme-secondary">操作说明</p>
            <ul class="mt-3 space-y-2 list-disc pl-5 text-sm text-theme-secondary">
      ...
      @if (session('success'))
        <div class="mt-6 rounded-xl border border-[rgb(var(--theme-primary))]/30 bg-[rgb(var(--theme-primary))]/10 p-3 text-sm text-[rgb(var(--theme-primary))]">
          {{ session('success') }}
        </div>
      @endif
      ...
      @if ($isGuest)
        <div class="mt-6 rounded-2xl border border-theme bg-theme-card p-5">
          <h2 class="text-base font-semibold text-theme">快速注册</h2>
          <p class="mt-2 text-sm text-theme-secondary">你当前是访客态，设置密码后即可将临时账号升级为正式账号。</p>

          <div class="mt-4 flex flex-wrap items-center gap-3">
            <button id="open-activate-modal" class="rounded-lg bg-[rgb(var(--theme-primary))] px-4 py-2 text-sm font-semibold text-theme-secondary">设置密码并注册</button>
            <a href="/login" class="text-sm text-[rgb(var(--theme-primary))] underline underline-offset-4">已有账号？去登录</a>
          </div>
        </div>
      @elseif (count($assets) > 0)
        <form method="POST" action="/recharge/requests" enctype="multipart/form-data" class="mt-6 space-y-4 rounded-2xl border border-theme bg-theme-card p-5">
          ...
          <div>
            <label class="mb-1 block text-sm text-theme-secondary">联系账号（用于人工核对）</label>
            <p class="rounded-lg border border-theme bg-theme-secondary px-3 py-2 text-theme">{{ auth()->user()?->username ?? '--' }}</p>
          </div>

          <div>
            <label for="payment_amount" class="mb-1 block text-sm text-theme-secondary">付款金额</label>
          ...
          @error('asset_code')
            <p class="text-sm text-[rgb(var(--theme-rose))]">{{ $message }}</p>
          @enderror

          <div>
            <label for="receipt_image" class="mb-1 block text-sm text-theme-secondary">付款截图</label>
            <div class="flex items-center gap-3 rounded-lg border border-theme bg-theme-secondary px-3 py-2">
              <label for="receipt_image" class="cursor-pointer rounded-md border border-[rgb(var(--theme-primary))]/40 px-3 py-1.5 text-sm text-[rgb(var(--theme-primary))] hover:bg-[rgb(var(--theme-primary))]/10">
                选择文件
              </label>
              <span id="receipt-image-name" class="text-sm text-theme-secondary">未选择文件</span>
            </div>
          ...
          <div>
            <label for="user_note" class="mb-1 block text-sm text-theme-secondary">备注（可选）</label>
